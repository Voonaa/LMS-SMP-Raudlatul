...
<body class="bg-background text-on-background font-body-md" x-data="{ mobileMenuOpen: false }">
    ...
    <header class="lg:hidden flex justify-between items-center bg-surface-container-lowest px-4 py-3 shadow-sm sticky top-0 z-40">
        ...
        <img alt="Profile" class="w-8 h-8 rounded-full border border-primary object-cover" src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->username) }}&background=006948&color=fff&rounded=true" />
    </header>
    ...
    <aside :class="mobileMenuOpen ? 'translate-x-0 flex' : '-translate-x-full lg:translate-x-0'" class="w-64 h-screen fixed left-0 top-0 overflow-y-auto shadow-sm bg-surface-container-lowest hidden lg:flex flex-col border-r border-outline-variant z-50 transform transition-transform duration-300 ease-in-out">
        ...
        <nav class="flex-1 px-4 py-6 space-y-2">
            <a class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.dashboard') ? 'text-primary bg-surface-container-low border-l-4 border-primary rounded-r-lg font-bold' : 'text-on-surface-variant hover:bg-surface-container rounded-lg' }} font-label-lg text-label-lg transition-colors" href="{{ route('admin.dashboard') }}">
                <span class="material-symbols-outlined">dashboard</span>
                Dashboard
            </a>
            <a class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.user.*') ? 'text-primary bg-surface-container-low border-l-4 border-primary rounded-r-lg font-bold' : 'text-on-surface-variant hover:bg-surface-container rounded-lg' }} font-label-lg text-label-lg transition-colors" href="{{ route('admin.user.index') }}">
                <span class="material-symbols-outlined">manage_accounts</span>
                Kelola Pengguna
            </a>
            <a class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('admin.config.*') ? 'text-primary bg-surface-container-low border-l-4 border-primary rounded-r-lg font-bold' : 'text-on-surface-variant hover:bg-surface-container rounded-lg' }} font-label-lg text-label-lg transition-colors" href="{{ route('admin.config.index') }}">
        ...
        <div class="p-4 border-t border-surface-container">
            <a class="flex items-center gap-3 px-4 py-3 {{ request()->routeIs('profile.settings') ? 'text-primary bg-surface-container-low border-l-4 border-primary rounded-r-lg font-bold' : 'text-on-surface-variant hover:bg-surface-container rounded-lg' }} font-label-lg text-label-lg transition-colors mb-2" href="{{ route('profile.settings') }}">
                <span class="material-symbols-outlined">settings</span>
                Settings
            </a>
    ...
    <main class="lg:ml-64 min-h-screen pb-24 lg:pb-0">
        ...
        <header class="hidden lg:flex justify-between items-center w-full px-margin py-base sticky top-0 z-30 bg-surface shadow-sm">
            <div class="flex-1">
                <h2 class="font-headline-md text-headline-md font-extrabold text-primary">@yield('page_title', 'Dashboard Admin')</h2>
            </div>
            <div class="flex items-center gap-6">
                <div class="flex items-center gap-3 cursor-pointer pl-4 border-l border-surface-container">
                    ...
                    <img alt="User Avatar" class="w-10 h-10 rounded-full border-2 border-primary object-cover" src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->username) }}&background=006948&color=fff&rounded=true" />
                </div>
            </div>
        </header>

        <!-- Main Canvas -->
        <div class="p-4 lg:p-margin">
            @yield('content')
        </div>
    </main>
    ...
    <nav class="lg:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 py-2 bg-surface-container-lowest shadow-lg border-t border-outline-variant rounded-t-xl">
        <a class="flex flex-col items-center justify-center {{ request()->routeIs('admin.dashboard') ? 'text-primary' : 'text-on-surface-variant hover:text-primary' }} transition-colors py-1 w-16" href="{{ route('admin.dashboard') }}">
            <span class="material-symbols-outlined mb-1" style="{{ request()->routeIs('admin.dashboard') ? 'font-variation-settings:\'FILL\' 1;' : 'font-variation-settings:\'FILL\' 0;' }}">home</span>
            <span class="font-label-sm text-[10px]">Beranda</span>
        </a>
        <a class="flex flex-col items-center justify-center {{ request()->routeIs('admin.user.*') ? 'text-primary' : 'text-on-surface-variant hover:text-primary' }} transition-colors py-1 w-16" href="{{ route('admin.user.index') }}">
            <span class="material-symbols-outlined mb-1" style="{{ request()->routeIs('admin.user.*') ? 'font-variation-settings:\'FILL\' 1;' : 'font-variation-settings:\'FILL\' 0;' }}">manage_accounts</span>
            <span class="font-label-sm text-[10px]">User</span>
        </a>
        <a class="flex flex-col items-center justify-center {{ request()->routeIs('admin.config.*') ? 'text-primary' : 'text-on-surface-variant hover:text-primary' }} transition-colors py-1 w-16" href="{{ route('admin.config.index') }}">
            <span class="material-symbols-outlined mb-1" style="{{ request()->routeIs('admin.config.*') ? 'font-variation-settings:\'FILL\' 1;' : 'font-variation-settings:\'FILL\' 0;' }}">settings</span>
            <span class="font-label-sm text-[10px]">Sistem</span>
        </a>
