...
@section('title', 'Buat Kuis - Guru')
@section('page_title', 'Buat Kuis')

@section('content')
<div class="mb-lg flex flex-col md:flex-row md:items-end justify-between gap-4">
    <div>
        <h2 class="font-headline-xl text-headline-xl text-on-surface">Buat Kuis Baru</h2>
        <p class="font-body-lg text-body-lg text-on-surface-variant mt-2">Buat kuis baru secara manual. Anda bisa menambahkan soal setelah kuis dibuat.</p>
    </div>
...
<div class="bg-surface-container-lowest rounded-xl shadow-[0_2px_10px_0_rgba(0,105,72,0.05)] border border-surface-container p-6 max-w-2xl">
    <form action="{{ route('guru.kuis.store') }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <input type="text" name="judul" class="w-full border border-outline-variant bg-surface rounded-lg p-3 focus:ring-primary focus:border-primary text-on-surface" placeholder="Masukkan judul kuis" required>
        </div>
        <div class="mb-6">
            <label class="block text-sm font-semibold text-on-surface mb-2">Deskripsi</label>
            <textarea name="deskripsi" rows="4" class="w-full border border-outline-variant bg-surface rounded-lg p-3 focus:ring-primary focus:border-primary text-on-surface" placeholder="Deskripsi singkat tentang kuis..."></textarea>
        </div>
        <div class="mb-6">
